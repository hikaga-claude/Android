...
class MainActivity : Activity() {
    ...
    private fun showManualDialog() {
        ...
        listOf("1. カウンター操作", "2. 色の変更", "3. 集計・コピー", "4. ファイル操作", "5. 音声入力 対応辞典")
        ...
        item("エクスポート", "メニュー →「設定データをエクスポート」または「カウントデータをエクスポート」\n保存スロットの内容をJSON形式で書き出します\nAndroid 10以降：ダウンロードフォルダに保存後トーストで通知し共有シートを表示\nAndroid 9以前：共有シートのみ表示")
        item("インポート", "メニュー →「設定データをインポート」または「カウントデータをインポート」\nエクスポートしたJSONファイルを選択すると対応するスロットに上書き復元\n異なる種類のファイルを読み込んだ場合はエラーを表示")

        // 音声入力辞典
        section("音声入力 対応辞典")
        item("登録ネタ（26種）",
            "まぐろ・いか・えび・ぶり・たまご・さけ・たい・かつお・あなご・たこ・" +
            "あかみ・ちゅうとろ・おおとろ・ぐんかん・なっとう・ほたて・あゆ・あじ・" +
            "いわし・さんま・さわら・たら・ふぐ・さば\n\n" +
            "ひらがな・カタカナ・漢字（例：鮪→まぐろ）のいずれでも認識します\n" +
            "辞典にないネタ名はひらがな・カタカナで発音すれば認識できます")
